...
def distorted_inputs_test(all_hdf5_data,dataset_test,batch_size,whichFoldData):
  ...
  log.info("whichFoldData [%s]test: Using %d IDs from dataset", whichFoldData,len(data_id))

  # single operations
  with tf.name_scope('input_reading_images_labels'):
      input_ops['id'] = tf.train.string_input_producer(
          tf.convert_to_tensor(data_id),
          capacity=128
      ).dequeue(name='input_ids_dequeue')

      m, label = get_data(data_id[0],all_hdf5_data)

      def load_fn(id):
          # image [n, n], label: [m]
          image, label = get_data(id,all_hdf5_data)
          return (id,
...
def distorted_inputs(all_hdf5_data, whichFoldData,dataset_train, batch_size):
  """Construct distorted input for CIFAR training using the Reader ops.
  ...
  data_id = dataset_train[whichFoldData]
  log.info("whichFoldData [%s]: Using %d IDs from dataset", whichFoldData,len(data_id))
