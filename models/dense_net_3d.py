...
class GAN3D(object):
  def __init__(self, config,data_provider,all_train_dir,whichFoldData,is_train=True):
    ...
    self.dataset_name       = config.hdf5FileName
  ...
  def build_GAN(self):
    ...
    d_real_softmax, d_no_softmax_real=self._build_graph_D(self.x_vector, reuse=False)
    d_fake_softmax, d_no_softmax_fake = self._build_graph_D(self.fake_image, reuse=True)
    ...
    def build_loss(d_real, d_real_logits, d_fake, d_fake_logits, label, real_image, fake_image):
      ...
      GAN_loss = tf.reduce_mean(total_d_loss + g_loss)

      # Classification accuracy
      correct_prediction = tf.equal(tf.argmax(d_real[:, :-1], 1),
                                    tf.argmax(self.labels, 1))
      accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
      return s_loss, d_loss_real, d_loss_fake, total_d_loss, g_loss, GAN_loss, accuracy


    self.all_preds = d_real_softmax
    ...
    self.S_loss, self.d_loss_real, self.d_loss_fake, self.d_loss, self.g_loss, self.GAN_loss, self.accuracy = \
      build_loss(d_real_softmax, d_no_softmax_real, d_fake_softmax, d_no_softmax_fake, self.labels, self.x_vector, self.fake_image)
