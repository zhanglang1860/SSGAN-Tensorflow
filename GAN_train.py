...
def construct_train_dir(config):
    ...
    hyper_parameter_str = 'all8020_{}_lr_g_{}_d_{}_batchSize{}_maxIteration{}'.format(
        temp[0], config.learning_rate_g, config.learning_rate_d,
         config.batch_size, config.max_training_steps
    )

    config.prefix = 'depth{}_growthRate{}_reduce{}_model_type_{}_keepPro{}'.format(
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    data_provider = get_data_provider_by_path(config, dataset_train, dataset_test, all_hdf5_data_train, all_hdf5_data_test, 0)
    model = DenseNet3D(config,data_provider,all_train_dir,0)
