...
class Discriminator(object):
    def __init__(self, name, num_class, norm_type, is_train,split_dimension_core,tt_rank):
        self.name = name
    ...
    def __call__(self, input):
        with tf.variable_scope(self.name, reuse=self._reuse):
            ...
            num_channel = [128, 256, 512, 1024, 2048, 2048, 2048, 2048, 2048]
            ...
            for i in range(num_layer):
                ...
                _ = conv2dtensorNet(_, ch, self._is_train, info=not self._reuse,
                           norm=self._norm_type, name='conv{}'.format(i + 1),split_dimension_core=self.split_dimension_core,tt_rank=self.tt_rank)
            ...
            for index in range(9):
                _ = conv2dtensorNet(_, int(_.get_shape().as_list()[-1] / 2), self._is_train, k=1, s=1,
                           info=not self._reuse, norm=self._norm_type, name='conv{}'.format(i + index + 2),split_dimension_core=self.split_dimension_core,tt_rank=self.tt_rank)
            ...
            _ = conv2dtensorNet(_, self._num_class + 1, self._is_train, k=1, s=1, info=not self._reuse,
                       ...
                       name='conv{}'.format(i + index + 3),split_dimension_core=self.split_dimension_core,tt_rank=self.tt_rank)
            _ = tf.squeeze(_)
            if not self._reuse: 
