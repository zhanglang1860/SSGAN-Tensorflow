...
class DataProvider(DataProvider):
  def __init__(self, config, dataset_train, dataset_test, all_hdf5_data, whichFoldData):
    """
    Args:
    ...
    if config.train:
      self.train = Data('train', dataset_train[whichFoldData],
                        'std', self._sequence_length,
                        self._crop_size, config.num_class, config.queue_size,all_hdf5_data)
    if config.test:
      self.test = Data('test', dataset_test[whichFoldData],
                      'std', self._sequence_length,
                      self._crop_size, config.num_class, config.queue_size,all_hdf5_data)
